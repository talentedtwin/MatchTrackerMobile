...
const LiveMatchScreen = ({ route, navigation }) => {
    ...
    if (isTimerRunning) {
      ...
      timerRef.current = setInterval(() => {
        ...
        if (
          totalSeconds % 10 === 0 &&
          totalSeconds > 0 &&
          totalSeconds !== lastLoggedSecond
        ) {
          console.log(
            `⏱️ Timer at ${totalSeconds} seconds (elapsed: ${elapsedSeconds}s)`
          );
  ...
  useEffect(() => {
    const subscription = AppState.addEventListener("change", (nextAppState) => {
      if (nextAppState === "active") {
        ...
        console.log(
          "📱 App went to background - timer continues via timestamps"
        );
      }
    });
  ...
  const toggleTimer = () => {
    if (isTimerRunning) {
      ...
      if (timerStartTime) {
        ...
        console.log(
          `⏸️ Pausing timer - accumulated ${elapsed}s, total: ${
  ...
  const adjustMatchTime = (seconds) => {
    ...
    if (isTimerRunning && timerStartTime) {
      ...
      const newAccumulated = Math.max(
        0,
        accumulatedSeconds + currentElapsed + seconds
      );
  ...
  const updatePlayerRedCard = (playerId) => {
    setPlayerStats((prev) => {
      ...
      Alert.alert("Red Card", "Give player a direct red card?", [
  ...
  const handleFinishMatch = async () => {
    ...
    Alert.alert("Finish Match", "Save the match result?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Finish",
        onPress: async () => {
      ...
      <ScrollView style={styles.content}>
                  ...
                  <View style={styles.badgeContent}>
                    ...
                    <Text
                      style={[
                        styles.badgeText,
                        {
                          color: theme.isDarkMode ? theme.text : COLORS.primary,
                        },
                      ]}
                    >
                      Cup
        ...
        >
          ...
          <View style={styles.timerDisplay}>
            ...
            <View style={styles.timerValue}>
              <Text style={[styles.timerTime, { color: theme.primary }]}>
        ...
        {selectedPlayers.length > 0 && (
          <>
            ...
            {playingPlayers.size > 0 && (
              ...
              >
                <View style={styles.sectionHeader}>
                  <View
                    style={[styles.statusIndicator, styles.statusPlaying]}
                  />
                      ...
                      >
                        ...
                        <View style={styles.playerSecondLine}>
                          ...
                          <TouchableOpacity
                            ...
                            onLongPress={() =>
                              removePlayerYellowCard(player.id)
                            }
                          >
            ...
            {/* Bench Players */}
            {selectedPlayers.filter((player) => !playingPlayers.has(player.id))
              .length > 0 && (
              <View
                style={[
                  styles.playersCard,
              ...
              >
                <View style={styles.sectionHeader}>
                  ...
                  <Text style={[styles.sectionTitle, { color: theme.text }]}>
                    ON BENCH (
                    {
                      selectedPlayers.filter((p) => !playingPlayers.has(p.id))
                        .length
                    }
                    )
                  </Text>
